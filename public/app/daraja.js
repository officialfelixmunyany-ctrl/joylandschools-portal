...
const UI = {
  toast(msg, type = ''){
    ...
    const tone = type === 'error' ? 'danger' : type === 'success' ? 'ok' : (type || 'ok');
    const icon = tone === 'danger' ? 'triangle-alert' : tone === 'warn' ? 'circle-alert' : tone === 'ok' ? 'check-circle' : 'info';
    t.className = 'toast ' + tone;
    t.setAttribute('role', tone === 'danger' ? 'alert' : 'status');
    t.setAttribute('aria-live', tone === 'danger' ? 'assertive' : 'polite');
    t.innerHTML = '<i data-lucide="' + icon + '" aria-hidden="true"></i><span>' + esc(msg) + '</span>';
    wrap.appendChild(t);
    this.paint();
...
async function boot(){
  await loadBrand();
  const forceSignin = new URLSearchParams(location.search).get('signin') === '1';
  let me = null;
  if (!forceSignin) {
  ...
  state.me = me;
  revealAndHide();
  if (forceSignin) return renderLogin();
...
function renderLogin(){
  ...
  screen.innerHTML = `
    <div class="scroll no-nav login-screen">
      <div class="appbar login-appbar">
        <a class="appbar-btn" href="/" aria-label="Back to main portal"><i data-lucide="arrow-left"></i></a>
        <div class="appbar-title">Teacher, Learner, Parent Portal</div>
      </div>
      <div class="login-wrap">
        <div class="login-intro">
          ...
          <div>
            <div class="section-label">Role portal login</div>
            <h2>${esc(school.school_name || 'Daraja')}</h2>
            <p>Sign in first, then open your teacher, learner, or parent dashboard.</p>
          </div>
        </div>
        ...
        <div class="login-assurance">
          <span><i data-lucide="presentation"></i> Teacher dashboard</span>
          <span><i data-lucide="graduation-cap"></i> Learner dashboard</span>
          <span><i data-lucide="users"></i> Parent dashboard</span>
        </div>
...
const TeacherApp = {
  ...
  marksGrid(classId, subjectId, className, subjectName){
    ...
    Router.push(async (s) => {
      ...
      const termName = marks.term ? (marks.term.term_number === 1 ? 'FIRST' : marks.term.term_number === 2 ? 'SECOND' : marks.term.term_number === 3 ? 'THIRD' : 'Term ' + marks.term.term_number) : 'SECOND';
      s.innerHTML = appbar({ title:subjectName, back:true }) + `<div class="scroll pad" style="padding-bottom:90px;">
        <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;">
          <div style="display:flex;flex-direction:column;gap:4px;">
            <span style="font-size:10px;font-weight:700;color:var(--ink-3);text-transform:uppercase;letter-spacing:.06em;">Term</span>
            <span style="font-family:var(--display);font-weight:600;font-size:15px;color:var(--ink);">${termName}</span>
          </div>
          <div style="width:1px;height:36px;background:var(--line);"></div>
          <div style="display:flex;flex-direction:column;gap:4px;flex:1;">
            <span style="font-size:10px;font-weight:700;color:var(--ink-3);text-transform:uppercase;letter-spacing:.06em;">Assessment</span>
            <div class="seg" id="mk-assess" style="margin:0;">
